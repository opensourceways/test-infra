...
import (
	...
	"k8s.io/test-infra/prow/gitee-plugins/close"
...
func initPlugins(agent *plugins.ConfigAgent, pm plugins.Plugins, cs *clients) {
	...
	v = append(v, reopen.NewReopen(gpc, cs.giteeClient, cs.ownersClient))
	v = append(v, close.NewClose(gpc, cs.giteeClient, cs.ownersClient))
	for _, i := range v {
		name := i.PluginName()
